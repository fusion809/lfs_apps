...
post_virtiofs_dir <- file.path(out_dir, "post-virtiofs")
dir.create(post_virtiofs_dir, showWarnings = FALSE)

cutoff_date <- as.POSIXct("2026-03-01 18:00:00")
mtimes <- file.info(files)$mtime

# Identify outliers
outlier_idx <- which(times < lower | times > upper)

# Identify post-virtiofs normal files
post_virtiofs_idx <- which(times >= lower & times <= upper & mtimes > cutoff_date)

outlier_files <- files[outlier_idx]
post_virtiofs_files <- files[post_virtiofs_idx]
...
if (length(outlier_files) > 0) {
  ...
  cat("\nMoved", length(outlier_files), "outlier files to", out_dir, "\n")
}

# Move post-virtiofs normal files
if (length(post_virtiofs_files) > 0) {
  file.rename(post_virtiofs_files,
              file.path(post_virtiofs_dir, basename(post_virtiofs_files)))
  cat("Moved", length(post_virtiofs_files), "normal post-virtiofs files to", post_virtiofs_dir, "\n")
}

EOF

grep -H --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox,.venv,venv} kernel ~/plots/outliers/post-virtiofs/*.svg 2>/dev/null | grep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox,.venv,venv} user | sed 's/.*= //g' > ~/lfs-scripts/boots.dat
